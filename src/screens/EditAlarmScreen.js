...
export default function EditAlarmScreen({ route, navigation }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.warn('Could not schedule alarm:', e.message);
    }
    navigation.goBack();
  };
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 11, color: '#666', marginBottom: 8, marginTop: 20, textTransform: 'uppercase' },
  input: { backgroundColor: '#151520', borderRadius: 10, padding: 12, color: '#fff', fontSize: 14, borderWidth: 1, borderColor: '#2a2a3a' },
  dayRow: { flexDirection: 'row', justifyContent: 'space-between' },
  dayChip: { width: 38, height: 38, borderRadius: 19, alignItems: 'center', justifyContent: 'center', backgroundColor: '#151520', borderWidth: 1, borderColor: '#2a2a3a' },
  ...
  challengeBox: { backgroundColor: '#151520', borderRadius: 12, padding: 14, borderWidth: 1, borderColor: '#2a2a3a' },
  challengeName: { fontSize: 15, color: '#fff', marginBottom: 4 },
  challengeDesc: { fontSize: 12, color: '#888', lineHeight: 18 },
  saveBtn: { marginTop: 28, backgroundColor: '#1D9E75', borderRadius: 12, padding: 14, alignItems: 'center' },
